...
// ============================================================================
// DATOS REALES DE INSTAGRAM (Sincronizado con la captura final de Emperador)
// ============================================================================
const INSTA_REELS = [
  { id: "i1", likes: "1.2k", comments: "45", type: "pinned", media_url: "https://images.unsplash.com/photo-1534158914592-062992fbe900?q=80&w=600&h=600&auto=format&fit=crop" }, // Mesa de pool (Fijado)
  { id: "i2", likes: "890", comments: "32", type: "pinned", media_url: "https://images.unsplash.com/photo-1560066984-138dadb4c035?q=80&w=600&h=600&auto=format&fit=crop" }, // Staff chicas (Fijado)
  { id: "i3", likes: "2.1k", comments: "112", type: "pinned", media_url: "https://images.unsplash.com/photo-1554519934-e32b1629d9ce?q=80&w=600&h=600&auto=format&fit=crop" }, // Clienta Bulls (Fijado)
  { id: "i4", likes: "5.4k", comments: "214", type: "reel", media_url: "https://images.unsplash.com/photo-1532635241-17e820acc59f?q=80&w=600&h=600&auto=format&fit=crop" }, // Chico lentes (Reel)
  { id: "i5", likes: "3.8k", comments: "145", type: "reel", media_url: "https://images.unsplash.com/photo-1517832606299-7ae9b620a186?q=80&w=600&h=600&auto=format&fit=crop" }, // Chico Arsenal (Reel)
  { id: "i6", likes: "4.2k", comments: "189", type: "reel", media_url: "https://images.unsplash.com/photo-1622286342621-4bd786c2447c?q=80&w=600&h=600&auto=format&fit=crop" }, // Corte Fade/cuello (Reel)
];
...
const DistributedVideo = ({ src, onUnmute, className }: { src: string, onUnmute: (vid: HTMLVideoElement) => void, className?: string }) => {
  ...
  // Forzar políticas de autoplay para asegurar que mesadepool.mp4 inicie
  useEffect(() => {
    if (videoRef.current) {
...
export default function UltimateEmperadorLanding() {
  ...
  // Al haber eliminado el MusicPlayer local, esta función ahora solo asegura 
  // que si un video se desmutea, los otros videos de la página se muteen.
  ...
  // ============================================================================
  // FETCH Y SLIDERS (Conectado con el Admin)
  // ============================================================================
  useEffect(() => {
  ...
  return (
    <main className="bg-[#050505] min-h-screen font-sans selection:bg-amber-500 selection:text-black overflow-x-hidden relative -mt-24 md:-mt-28">
      ...
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1, type: "spring", stiffness: 100 }} className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[100]">
      ...
      {/* ========================================================================= */}
      {/* 1. HERO GLOBAL DINÁMICO (Cero Filtros, Texto Limpio sin cortes) */}
      {/* ========================================================================= */}
      <section className="relative w-full h-[100dvh] flex flex-col justify-center overflow-hidden bg-[#050505]">
          ...
          <motion.div key={currentHeroSlide} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.5, ease: "easeInOut" }} className="absolute inset-0 w-full h-full flex flex-col justify-center">
            ...
            <div className="w-full h-full relative flex flex-col justify-center items-center">
              <motion.div style={{ y: yHero }} className="absolute inset-0 w-full h-full z-0">
                {/* IMAGEN SIN FILTROS OSCUROS y sin gradiente encima */}
                <MediaRenderer 
        ...
        <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 text-center pt-[150px] md:pt-[220px] pointer-events-none flex flex-col justify-center h-full">
          {currentSlideData?.type !== 'promo' ? (
            /* RENDER BRANDING (Eliminado fondo oscuro bg-black/30 del contenedor de texto) */
            <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="pointer-events-auto inline-block self-center">
              ...
              {/* Eliminado overflow-hidden que cortaba el texto y modificado leading a leading-none con padding inferior */}
              <div className="mb-2">
      ...
      {/* ========================================================================= */}
      {/* 7. INSTAGRAM (AMPLIADO, ACTUALIZADO Y EXACTO AL INSTAGRAM REAL) */}
      {/* ========================================================================= */}
      <section id="instagram" className="py-24 md:py-32 bg-[#0a0a0a] relative overflow-hidden border-b border-zinc-900 flex justify-center">
         ...
         {/* FIX APLICADO: Max-width ampliado a 800px para que se vea imponente */}
         <div className="w-full max-w-[800px] px-6 relative z-10">
            
            {/* CABECERA EXTERIOR (Abrir App) */}
            <a href="https://www.instagram.com/emperador_barbershop/?hl=es" target="_blank" rel="noopener noreferrer" className="flex justify-between items-center mb-6 text-white font-bold text-base px-2 group cursor-pointer">
              <div className="flex items-center gap-3">
                <div className="bg-gradient-to-tr from-[#fdf497] via-[#fd5949] to-[#d6249f] rounded-[0.5rem] p-1.5 group-hover:scale-110 transition-transform">
                  <Instagram size={24} className="text-white" />
                </div>
                <span className="tracking-wide text-lg md:text-xl group-hover:text-amber-500 transition-colors">@emperador_barbershop</span>
              </div>
              <span className="text-xs md:text-sm uppercase font-black tracking-widest text-zinc-400 group-hover:text-white transition-colors">
                ABRIR APP ↗
              </span>
            </a>

            {/* TARJETA BLANCA GIGANTE */}
            <div className="bg-white rounded-[2.5rem] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.5)] hover:shadow-[0_20px_60px_rgba(214,36,159,0.2)] transition-shadow duration-500 cursor-pointer">
               
               {/* Header Perfil Ampliado */}
               <a href="https://www.instagram.com/emperador_barbershop/?hl=es" target="_blank" rel="noopener noreferrer" className="block p-6 md:p-10 relative group">
                  <div className="absolute top-8 right-8 group-hover:scale-110 transition-transform">
                     <Instagram size={36} className="text-[#d6249f]" />
                  </div>

                  <div className="flex items-center gap-6 md:gap-8">
                     {/* Borde Instagram en el Logo Ampliado */}
                     <div className="shrink-0 w-[100px] h-[100px] md:w-[140px] md:h-[140px] rounded-full bg-gradient-to-tr from-[#fdf497] via-[#fd5949] to-[#d6249f] p-[4px] md:p-[5px] shadow-md group-hover:scale-105 transition-transform">
                        <div className="w-full h-full bg-white rounded-full p-[3px] md:p-[4px]">
                           <div className="w-full h-full relative rounded-full overflow-hidden border border-gray-200 bg-black flex items-center justify-center">
                             <Image src="/logo.png" fill className="object-cover p-2" alt="Emperador Logo Instagram" />
                           </div>
                        </div>
                     </div>
                     <div className="text-black flex flex-col justify-center">
                        <h3 className="font-bold text-2xl md:text-3xl leading-none mb-2 group-hover:text-[#d6249f] transition-colors">emperador_barbershop</h3>
                        <p className="text-zinc-600 text-base md:text-lg mb-3 font-medium">Emperador BarberShop</p>
                        
                        {/* Datos actualizados */}
                        <div className="flex flex-wrap items-center gap-4 md:gap-6 mt-1">
                           <p className="text-zinc-900 text-[15px] md:text-base"><span className="font-black">84</span> publicaciones</p>
                           <p className="text-zinc-900 text-[15px] md:text-base"><span className="font-black">1039</span> seguidores</p>
                           <p className="text-zinc-900 text-[15px] md:text-base"><span className="font-black">488</span> seguidos</p>
                        </div>
                     </div>
                  </div>
               </a>

               {/* FIX APLICADO: GRID DE FOTOS EXACTAS A TU CAPTURA DE PANTALLA */}
               <div className="grid grid-cols-3 gap-1 md:gap-1.5 bg-gray-200">
                  {reels.map((post) => (
                    <a 
                      key={post.id} 
                      ...
                      className="relative aspect-square bg-zinc-300 overflow-hidden group block"
                    >
                       <Image 
                         src={post.media_url} 
                         alt="Insta Post" 
                         ...
                         unoptimized
                       />
                       {/* Iconos de Pin y Reel actualizados para igualar a Instagram */}
                       <div className="absolute top-3 right-3 text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] z-10">
                          {post.type === 'pinned' && <Pin size={20} fill="currentColor" className="rotate-45" />}
                          {post.type === 'reel' && <PlayCircle size={20} fill="currentColor" />}
                       </div>
                       
                       <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-3 backdrop-blur-sm z-20">
                          <div className="flex items-center gap-2 text-white font-black text-lg md:text-xl"><Heart fill="currentColor" size={24} /> {post.likes}</div>
                          <div className="flex items-center gap-2 text-white font-black text-lg md:text-xl"><MessageCircle fill="currentColor" size={24} /> {post.comments}</div>
                       </div>
                    </a>
                  ))}
               </div>
